...
from .plain_mamba_layer import PlainMambaLayer
from .custom_avg_pooling import CustomAveragePooling
...
class PlainMamba(BaseBackbone):
    ...
    def __init__(self,
        ...
        self.neck = CustomAveragePooling()
        self.head = LinearClsHead(num_classes= num_classes, in_channels= embed_dims)

        # self.layer_head = LinearClsHead(num_classes= num_classes, in_channels= embed_dims)

        # # For semantic alignment
        # self.align_mlp = LinearClsHead(num_classes = 384, in_channels = 384, bias= False)
        ...
        # self.norm_layer = build_norm_layer(norm_cfg, self.embed_dims)[1]
    ...
    def forward(self, x):
        ...
        outs = []
        # only for all layer distill
        outputs = []
        for i, layer in enumerate(self.layers):
            x = layer(x, hw_shape=patch_resolution)
            tem_x = self.neck((x, outs))
            # outputs.append((tem_x - tem_x.mean()) / (tem_x.std() + 1e-8))
            # outputs.append(self.layer_head(tem_x))
            outputs.append(tem_x)
        ...
        # # zero-padding, middle
        # pad = nn.ZeroPad2d((96,96,0,0))
        # out = pad(out)

        # # zero-mean and unit variance
        # out = (out - out.mean()) / (out.std() + 1e-8)
    
        # out = self.align_mlp(out)
        return x, out, outputs