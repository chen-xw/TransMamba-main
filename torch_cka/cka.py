...
class CKA:
    ...
    def compare(self,
        ...
        N = 12#len(self.model1_layers) if self.model1_layers is not None else len(list(self.model1.modules()))
        M = 12#len(self.model2_layers) if self.model2_layers is not None else len(list(self.model2.modules()))
        ...
        for (x1, *_), (x2, *_) in tqdm(zip(dataloader1, dataloader2), desc="| Comparing features |", total=num_batches):
            ...
            self.model1_features = {}
            self.model2_features = {}
            out1 = self.model1(x1.to(self.device))
            out2 = self.model2(x2.to(self.device))
            self.model1_features = {}
            self.model2_features = {}
            for i in range(len(out1[2])):
                self.model1_features['layer_{}'.format(i)] = out1[2][i]
                self.model2_features['layer_{}'.format(i)] = out2[2][i]
            # self.model1_features['feature'] = out1[1]
            # self.model2_features['feature'] = out2[1]
    ...
    def plot_results(self,
                     save_path: str = 'Mamba DeIT cifar.jpg',
                     title: str = None):
        fig, ax = plt.subplots()
