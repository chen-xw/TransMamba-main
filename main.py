...
from RWKV_models import build_model
from rwkv_config import rwkv_get_config
...
def get_args_parser():
    ...
    parser.add_argument('--model-type', default='PlainMamba', type=str,choices=['PlainMamba', 'Mamba', 'DeiT', 'DeiT-Finetune', 'VMamba', 'ViM', 'rkwv'])
    ...
    # Model parameters
    parser.add_argument('--model', default='vim_tiny', type=str, metavar='MODEL',
                        help='Name of model to train')   #  deit_small_patch16_224
    parser.add_argument('--model-teacher', default='deit_tiny_patch16_224', type=str, metavar='MODEL',
                        help='Name of model to train')   #  deit_small_patch16_224
    parser.add_argument('--model-path', default='', type=str, metavar='MODEL',
    ...
    # Dataset parameters   /data2/xiuwei/Datasets/ImageNet1k/images /data2/xiuwei/Datasets/imagenet100
...
def main(args):
    ...
    rwkv_config = rwkv_get_config(args)
    ...
    #     class_sample = np.random.choice(class_indices[i], size=int(len(class_indices[i]) // class_counts), replace=False)
    ...
    elif args.model_type =='DeiT' or args.model_type =='DeiT-Finetune' or args.model_type =='ViM':
        model = create_model(
            args.model,
            pretrained=False,
            num_classes=args.nb_classes,
    ...
    checkpoint_deit = torch.load('/data2/xiuwei/Code/deit-main/checkpoint/DeiT-Finetune_IMNET_224_checkpoint_best.pth')
    ...
    # for i, block in enumerate(model_deit.blocks):
    #     block.register_forward_hook(hook_fn_teacher)
    # Student Model Checkpoint for Finetune
    # checkpoint = torch.load('/data3/xiuwei/Code/deit-main/checkpoint/deit_base_patch16_224-b5f2ef4d.pth')
    ...
    criterion = DistillationLoss(
        criterion, model_deit, args.distillation_type, args.distillation_alpha, args.distillation_tau
    )
    ...
    for epoch in range(args.start_epoch+1, args.epochs+1):
        ...
        if args.output_dir and epoch %50 ==0 :
            checkpoint_paths = [output_dir / '{}_{}_{}_checkpoint_{}.pth'.format(args.model_type,args.data_set,args.input_size,epoch)]
        ...
        if max_accuracy < test_stats["acc1"]:
            ...
            if args.output_dir:
                checkpoint_paths = [output_dir / '{}_{}_{}_checkpoint_best.pth'.format(args.model_type,args.data_set,args.input_size)]
                for checkpoint_path in checkpoint_paths:
                    utils.save_on_master({
                        'model': model_without_ddp.state_dict(),
                        'optimizer': optimizer.state_dict(),
